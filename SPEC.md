...
- 2026-07-18: Optimized `serialize_model` in `mjcf_helpers.py` by removing `ET.indent()` and moving the indentation logic directly into `_fast_serialize_node`. This avoids the expensive O(N) tree traversal and redundant string allocation into `.text` and `.tail` attributes.
