...
- 2026-06-22: Removed `xml.etree.ElementTree.indent()` usage and incorporated indentation natively into the recursive XML string building loop (`_fast_serialize_node`) in `mjcf_helpers.py` to eliminate an unnecessary O(N) tree traversal prior to string formatting.
