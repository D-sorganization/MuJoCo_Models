...
- 2026-06-19: Optimized `_fast_serialize_node` in `mjcf_helpers.py` by replacing `f-strings` with multiple `buffer.append()` calls and caching `bool(len(elem))` to avoid evaluating iterators on empty leaf nodes, significantly speeding up large model MJCF XML string generation.
- 2026-06-19: Optimized \_fast_serialize_node in mjcf_helpers.py by replacing f-strings with multiple buffer.append() calls and caching bool(len(elem)) to avoid evaluating iterators on empty leaf nodes, significantly speeding up large model MJCF XML string generation.
...
- 2026-07-18: Optimized `serialize_model` in `mjcf_helpers.py` by integrating XML formatting (indentation) directly into the single-pass `_fast_serialize_node` recursive descent, entirely eliminating the redundant O(N) tree traversal previously caused by `ET.indent()`.
- 2026-07-18: Updated `pyproject.toml` mypy configuration to use `python_version = "3.12"` to prevent syntax errors when analyzing typing in dependencies like numpy that use modern type statements in their stub files.
