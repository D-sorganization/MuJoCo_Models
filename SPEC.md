...
- 2026-07-15: Optimized custom XML serialization in `mjcf_helpers.py`. Eliminated tuple allocations by replacing `buffer.extend(tuple(...))` with successive `buffer.append()` calls and inlined character escaping using chained `.replace()` calls instead of delegating to standard library functions to avoid call frame overhead.
