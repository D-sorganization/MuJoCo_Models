...
- 2024-11-20: Optimized XML escaping in `_fast_serialize_node` in `mjcf_helpers.py` by completely replacing `xml.etree.ElementTree._escape_attrib` and `_escape_cdata` calls with inline `.replace()` chains, which bypassed standard library python frame call overhead entirely for a ~15% performance gain.
