...
- 2026-07-16: Removed `xml.etree.ElementTree.indent()` from MJCF serialization logic, integrating indentation directly into the `_fast_serialize_node` recursive pass to avoid an O(N) tree traversal prior to string construction.
- 2026-07-16: Updated `python_version` in `[tool.mypy]` to `3.12` to fix syntax errors caused by NumPy 2.x using `type` statements which are unsupported in Python < 3.12.
