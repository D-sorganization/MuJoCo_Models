...
- 2026-07-14: Optimized `_fast_serialize_node` in `mjcf_helpers.py` by replacing `list.extend` calls with sequential `list.append` calls and replacing standard library `xml.etree.ElementTree._escape_attrib` and `_escape_cdata` calls with inline `.replace()` chains inside the fast-path checks.
...
Update hash for CI: cec466d3e35e5ce2e16060afa7a7cfe0
