...
- Python test functions (`def test_*`): 418
...
| `src/mujoco_models/exercises/base.py` | `build` | 46 |
| `src/mujoco_models/__main__.py` | `_build_parser` | 45 |
| `src/mujoco_models/__main__.py` | `main` | 44 |
...
**Finding**: 5 `print(...)` call(s) in `src/` — should use `logging`. Violates CI rule in repos that enforce no-print.
...
### TDD
- 418 test functions across 28 test files.
...
1. **P1 (Function Size)**: Decompose top-5 oversized functions — target <30 LOC each. Keep single responsibility per function.
   ...
   - `src/mujoco_models/exercises/base.py::build` (46 LOC)
   - `src/mujoco_models/__main__.py::_build_parser` (45 LOC)
   - `src/mujoco_models/__main__.py::main` (44 LOC)
...
3. **P1 (Logging)**: Replace 5 `print()` call(s) in `src/` with `logging` module calls.
