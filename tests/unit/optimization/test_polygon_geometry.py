...
class TestSquaredDistanceToPolygon:
    ...
    def test_point_to_segment_clamping(self) -> None:
        ...
        dist = squared_distance_to_polygon(
            np.array([-1.0, 0.0]), np.array([[0.0, 0.0], [1.0, 0.0], [0.5, 1.0]])
        )
        assert dist == pytest.approx(1.0)

        # Test t > 1.0 branch
        dist = squared_distance_to_polygon(
            np.array([2.0, 0.0]), np.array([[0.0, 0.0], [1.0, 0.0], [0.5, 1.0]])
        )
    ...
    def test_point_to_segment_clamping_zero(self) -> None:
        ...
        dist = squared_distance_to_polygon(
            np.array([2.0, 0.0]), np.array([[0.0, 0.0], [1e-13, 0.0], [0.0, 1.0]])
        )
