...
## 2026-06-25 - Avoid ElementTree.indent prior to custom XML serialization

**Learning:** Calling `xml.etree.ElementTree.indent()` prior to custom recursive XML serialization causes a redundant O(N) tree traversal just to append whitespace text/tail nodes. This adds significant unnecessary overhead (~10%) during large XML model generation.
**Action:** Track a `level` parameter during the single serialization descent (e.g., inside `_fast_serialize_node`) and manually output whitespace indentation directly to the string buffer. When doing so, ignore purely whitespace `.text` and `.tail` nodes (using `.isspace()`) to avoid double-indenting pre-formatted trees.
