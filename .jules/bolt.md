...
## 2026-07-16 - Inline XML Indentation to avoid O(N) Traversal
**Learning:** Calling `xml.etree.ElementTree.indent()` prior to custom recursive XML serialization causes a redundant O(N) tree traversal just to append whitespace nodes. It takes about 15-20% of the execution time.
**Action:** When manually implementing fast custom recursive serialization loops for XML strings, track a `level` parameter during the single serialization descent and manually output indentation to the string buffer. Avoid double-indenting pre-formatted trees by ignoring purely whitespace `.text` and `.tail` nodes.
