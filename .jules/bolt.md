...
## 2026-07-04 - XML ElementTree redundant indentation traversal

**Learning:** Calling `xml.etree.ElementTree.indent()` prior to custom recursive XML serialization (like `_fast_serialize_node`) causes a completely redundant O(N) tree traversal just to append whitespace to `.tail` and `.text` attributes.
**Action:** Instead of making a separate pass with `ET.indent()`, track a `level` parameter during the single recursive serialization descent and manually output indentation (`\n` + `"  " * level`) to the string buffer. Be sure to `.strip()` `.text` and `.tail` of existing whitespace to avoid double-indenting pre-formatted trees.
