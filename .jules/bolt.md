...
## 2026-07-15 - [Inline XML Indentation]
**Learning:** Avoid calling `xml.etree.ElementTree.indent()` prior to custom recursive XML serialization, as it causes a redundant O(N) tree traversal just to append whitespace.
**Action:** Track a level parameter during the single serialization descent and manually output indentation to the string buffer to avoid double passes over the tree.
