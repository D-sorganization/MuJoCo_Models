...
## 2026-07-02 - Fast XML Serialization

**Learning:** When manually implementing fast custom recursive serialization loops for XML strings, bypass standard library `xml.etree.ElementTree._escape_attrib` and `_escape_cdata` entirely. Use a fast-path Python check (`if '&' in v or '<' in v:`) followed by inline `.replace()` operations, which is significantly faster than the standard library function call overhead for escaping. Furthermore, append elements directly to the list buffer via multiple `buffer.append()` calls instead of using `list.extend` with inline tuples to avoid significant tuple allocation overhead.

**Action:** In custom string generation/serialization loops, avoid `list.extend` with tuples and inline string replacements when possible.
