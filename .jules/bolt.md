...
## 2026-07-05 - Avoid tuple and `buffer_extend` in recursive string builders
**Learning:** In tight custom recursive string builders (such as `_fast_serialize_node` for XML serialization), passing multiple items to `buffer_extend` using an inline tuple like `buffer_extend((" ", k, '="', v, '"'))` causes unnecessary tuple allocation overhead on every node. Also, calling wrapper functions for XML escaping (`_escape_attrib`) introduces function call overhead in tight loops.
**Action:** Replace `buffer_extend` and tuple usage with multiple consecutive `buffer_append` calls, and inline simple `.replace()` chains inside the fast-path checks for XML escaping.
