...
def ensure_valid_xml(xml_string: str) -> ET.Element:
    ...
    Raises ValidationError if the string is not well-formed XML.
    """
    try:
        return ET.fromstring(xml_string)  # nosec B314 — parsing self-generated XML
    except ET.ParseError as exc:
        raise ValidationError(f"Generated XML is not well-formed: {exc}") from exc


def ensure_mjcf_root(root_or_xml: str | ET.Element) -> ET.Element:
    """Verify the root element is ``<mujoco>``.

    Raises ValidationError if the XML is malformed or the root tag is wrong.
    """
    if isinstance(root_or_xml, str):
        ...
        root = root_or_xml
    if root.tag != "mujoco":
        raise ValidationError(f"MJCF root must be <mujoco>, got <{root.tag}>")
    return root
...
def ensure_positive_definite_inertia(
    ...
    if ixx <= 0:
        raise ValidationError(f"Postcondition violated: {body_name} Ixx={ixx} not positive")
    if iyy <= 0:
        raise ValidationError(f"Postcondition violated: {body_name} Iyy={iyy} not positive")
    if izz <= 0:
        raise ValidationError(f"Postcondition violated: {body_name} Izz={izz} not positive")
